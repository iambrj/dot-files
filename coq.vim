...
if !exists('b:coqtail_did_highlight') || !b:coqtail_did_highlight
  function! s:CoqtailHighlight() abort
    ...
    elseif &t_Co > 16
      hi def CoqtailChecked ctermbg=27 guibg=Grey27
      hi def CoqtailSent ctermbg=60 guibg=CornflowerBlue
    else
      hi def CoqtailChecked ctermbg=4 guibg=Grey27
      hi def CoqtailSent ctermbg=7 guibg=CornflowerBlue
    endif
...
syn region coqImplicitOption      contained matchgroup=coqVernacCmd start="\<\%(\%(Local\|Global\)\_s\+\)\?Arguments\>" end="\.\_s"
...
" Coercions
syn region coqCoercion      contains=coqCoercionName,coqCoercionBody1,coqCoercionBody2 matchgroup=coqVernacCmd start="\<\%(\%(Local\|Global\)\_s\+\)\?\%(Identity\_s\+\)\?Coercion\>" matchgroup=coqVernacPunctuation end="\.\_s" keepend
